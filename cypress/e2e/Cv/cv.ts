...
Given("User visits cv page", () => {
  cy.visit(`${Cypress.config().baseUrl}/experience`);
});
...
Then("Child cv details will be hidden", () => {
  cy.get('[data-cy="cv-parent-node"]:first')
    .next('[data-cy="cv-child-node"]')
    .should("not.exist");
});
...
Then("User will load cv detail", () => {
  cy.location("pathname").should("eq", "/experience/solutions-developer");
});
